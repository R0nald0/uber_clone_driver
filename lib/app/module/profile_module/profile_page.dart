...
class _ProfilePageState extends State<ProfilePage> with DialogLoader{
    ...
    return Scaffold(
      ...
      child: CustomScrollView(
        slivers: [
                ...
                showDialog(context: context, 
                 builder:(context) {
                  return  AlertDialog(
                    ...
                    title: const Text("Altere dados do perfil",textAlign: TextAlign.center,),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: Image.asset(UberCloneConstants.ASSEESTS_IMAGE_LOGO,fit: BoxFit.cover,)),
              ...
              title:  Observer(
                builder: (_) {
                  return InfoUserWidget(
                    driverName: widget.profileController.usuario?.nome ?? 'UserName' ,
                    tripFinalized: 3,
                    boxTitle:  [{'Corridas ${widget.profileController.requests?.length}' : false} , const {'1.0' : true}],
                  );
                }
              ),
              background: SizedBox(
                    ...
                    const Positioned(
                        ...
                        child: SizedBox(
                            ...
                            child: ProfileImageWidget(
                               urlImage: "https://images.unsplash.com/photo-1504593811423-6dd665756598?q=80&w=1470&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                            )))
                  ],
                ),
              ),
          ...
          CardBalanceWidget(
            balance:widget.profileController.usuario?.balance.toDouble() ?? 0.0
          ),
          SliverToBoxAdapter(child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16,vertical: 8),
            child: Text("Viagems Realizadas",
          ...
          Observer(
            builder: (context) {
              return Visibility(
                visible: widget.profileController.requests != null && widget.profileController.requests!.isNotEmpty ,
  ...
  Future<void> _initReaction()async {
      final userReaction = reaction<Usuario?>((_)=> widget.profileController.usuario, (user){
            showLoaderDialog();
            if (user != null) {
               hideLoader();
            }
     ...
     final errorReaction = reaction<String?>((_)=> widget.profileController.errorMessage, (error){
            if (error != null) {
               callSnackBar(error);
            }
        });

      reactionsDisposer.addAll([userReaction,errorReaction]);   
  }
}
